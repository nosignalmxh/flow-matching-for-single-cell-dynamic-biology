...
def plot_delta_distribution(
    ...
    *,
    ...
    for x, vals in enumerate(data):
        ...
        y_span = max(np.nanmax(vals) - np.nanmin(vals), 1e-6)
        ax.text(x, mean_val + 0.045 * y_span, f"mean {mean_val:.{int(mean_ndigits)}f}", ha="center", va="bottom", fontsize=7, color=palette["diagnostic"])
