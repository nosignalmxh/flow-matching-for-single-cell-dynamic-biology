...
def draw_fig5_1_panels(
    ...
    *,
    ...
    paths.update(draw_fig5_1_combined_panel(paths, fig_dir=fig_dir, display_fn=display_fn))
    return paths
...
def draw_fig5_1_combined_panel(
    ...
    *,
    ...
    configure_fig5_1_style()
    return _draw_combined(fig5_1_paths, Path(fig_dir), display_fn=display_fn)
...
def _crop_white_margin(img: np.ndarray, threshold: float = 0.985, pad: int = 18) -> np.ndarray:
    rgb = img[..., :3]
...
def _draw_combined(
    ...
    *,
    ...
    combined_inputs = {
        "Training time-pair designs": paths[f"{FIG5_1_STEMS['time_pair_designs']}_png"],
        "Hidden t=2 recovery": paths[f"{FIG5_1_STEMS['hidden_t2_recovery']}_png"],
        "Seen t=4 rollout": paths[f"{FIG5_1_STEMS['seen_t4_rollout']}_png"],
        "Velocity jump at hand-offs": paths[f"{FIG5_1_STEMS['velocity_jump']}_png"],
    }
    fig = plt.figure(figsize=(11.4, 7.0), facecolor="white")
    positions = [
        [0.02, 0.55, 0.47, 0.39],
        [0.53, 0.55, 0.45, 0.39],
        [0.02, 0.08, 0.45, 0.39],
    ...
    for path, position in zip(combined_inputs.values(), positions):
        ax = fig.add_axes(position)
        ax.imshow(_crop_white_margin(mpimg.imread(path)))
        ax.set_axis_off()
    return _save_display(fig, fig_dir, FIG5_1_STEMS["combined"], width=960, display_fn=display_fn)
