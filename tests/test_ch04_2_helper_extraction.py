...
def test_ch04_2_imports_extracted_helper_modules():
    from src.visualization.manifold_diagnostics import candidate_path_diagnostics, stats_for_selected_pairs
    from src.visualization.state_space import plot_delta_distribution, representative_sources_from_plan
...
def test_ch04_2_notebook_no_longer_defines_extracted_helpers_locally():
    nb = json.loads(Path("notebooks/chapter4_2_state_space_assumptions.ipynb").read_text())
    code_sources = [
    ...
    joined = "\n".join(code_sources)
    assert "from src.visualization.manifold_diagnostics import" in joined
    assert "from src.visualization.state_space import" in joined
