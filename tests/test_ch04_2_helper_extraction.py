...
def test_ch04_2_imports_extracted_helper_modules():
    from src.manifold_diagnostics import candidate_path_diagnostics, stats_for_selected_pairs
    from src.state_space_figures import plot_delta_distribution, representative_sources_from_plan
...
def test_ch04_2_notebook_no_longer_defines_extracted_helpers_locally():
    ...
    joined = "\n".join(code_sources)
    assert "from src.manifold_diagnostics import" in joined
    assert "from src.state_space_figures import" in joined
